...
export class TimerValveWorkerService {
  ...
  public async TimerProcess() {
    // Burada önce Gateway daha sonra workgroup, timer-management bilgileri ve en son timer-management-detail
    // Bilgilerini çekerek işlemlerimize başlıyoruz
    try {
      ...
      gateways.forEach(async (element) => {
        ...
        (await workGroups).forEach(async (workGroup) => {
          ...
          timerManagement.forEach(async (timer) => {
            ...
            // eğer haftalık planda vana otomatik yönetimdeyse otomatık çalıştır değilse sms,whatsapp,mail gönder ve log at
              ...
              if (detail.IsAuto) {
                detail.Days.forEach((day, index) => {
                  ...
                  if (
                    ...
                    // KARTA BAĞLAN VE SULAMA- VANA AÇMA KOMUTLARINI GÖNDER
                    ...
                    this.timerManagementLogService.create({
                      ...
                      LogContent: `${element.contentId}'li ${element.Name}'li Gateway Timer Yönetimi Başarılı Vana Açıldı!`,
                      LogDescription: `${element.Name} Bağlantı Sağlandı ve Timer Yönetimi Sağlandı`,
                      LogTitle: `${element.Name} Rutin İşlemler - Timer`,
                      LogStatus: 'Success',
                      contentId: '',
                      createdAt: new Date(),
                      isDeleted: false,
                  ...
                  this.timerManagementLogService.create({
                    TimerManagementID: timer.contentId,
                    LogContent: `${element.contentId}'li ${element.Name}'li Gateway Timer Yönetimi Başarısız Vana Açılamadı!`,
                    LogDescription: `${element.Name} Bağlantı Sağlandı ve Timer Yönetimi Sağlanamadı!!`,
                    LogTitle: `${element.Name} Rutin İşlemler - Timer`,
                    LogStatus: 'Error!!',
                    contentId: '',
                    createdAt: new Date(),
  ...
  public async connectToGateway(
    ...
    // Gateway IP PORT ve Komut ile birlikte bağlantı data atacağız.
    // GatewayCommand array'in her zaman ilk 5 indexi cihaz bilgilerini barındırmalıdır.
    return true;
  }
